<template>
  <Form ref="formValidate" :model="formValidate" :rules="ruleValidate" :label-width="120">
    ...
    <FormItem label="活动介绍" prop="description">
      <quill-editor v-model="formValidate.description" @onEditorBlur="getQuillContent" :content="formValidate.description"></quill-editor>
    </FormItem>
    <FormItem>
    ...
    handleSubmit(name, status) {
      this.$refs[name].validate(valid => {
        if (valid) {
          ...
          this.$axios.post('/admin/event', options).then(res => {
            if (status == 1) {
            ...
            this.$router.push({ path: '/activity/edit/' + res.data });
          });
        } else {
