<template>
    <div>
        <Form class="search-form" ref="searchForm" :model="searchForm" label-position="right" :label-width="70" inline>
            ...
            <Form-item label="认证状态" prop="cert_status">
                <Select v-model="searchForm.cert_status" filterable clearable placeholder="默认全部" @on-change="getUserLists" style="width: 141px;">
        ...
        <Modal v-model="addUserModalVisible" title="添加用户">
            ...
            <Form ref="addUserForm" :model="addUserForm" :rules="addUserFormRules" label-position="right" :label-width="80">
                ...
                <Form-item label="邮箱" prop="email">
                    <Input v-model="addUserForm.email" placeholder="请输入邮箱，可用于登录"></Input>
                </Form-item>
            ...
            list_columns: [
                ...
                },
                {
                    ...
                    render: (h, params) => {
                        ...
                        if (weibo) weibo_str = '昵称（' + weibo.nickname + '）；openid（' + weibo.openid + '）';
        ...
        addUser(name) {
            this.$refs[name].validate((valid) => {
                if (valid) {
                    ...
                    this.$axios.put('/admin/plinks/', this.addUserForm).then(res => {
                        if (res.code == 0) {
                            this.getUserLists();
